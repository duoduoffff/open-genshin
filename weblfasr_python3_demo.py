...
def documentWrite(fname, content):
    f = open(fname, "a")
    f.write(content)
    f.write("\n")
    f.close()
...
def recurringFiles():
    for file in os.listdir("."):
        if os.path.isfile(file) and file.endswith(".wav"):
            api = RequestApi(appid="5f7d65ea", secret_key="68b9fbe96cd40afeef4c56fd84f7d3e1", upload_file_path=file)
            ret = api.all_api_request()
            if ret['ok'] == 0:
            ...
            result = file + "," + s
            print(result)
            documentWrite("result.csv", result)
